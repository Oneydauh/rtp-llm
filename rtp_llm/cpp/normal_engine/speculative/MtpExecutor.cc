    ...
    for (auto& mtp_params : *propose_params->mtp_model_params_) {
        ...
        if (!params.py_sp_model.is_none()) {
            ...
            if (force_single_prefill_capture) {
                draft_model_params.exec_init_params.hw_kernel_config.enable_cuda_graph = false;
                draft_model_params.exec_init_params.hw_kernel_config.decode_capture_batch_sizes.clear();
                draft_model_params.exec_init_params.hw_kernel_config.prefill_capture_seq_lens.clear();
                RTP_LLM_LOG_INFO(
