...
class TorchAsyncEvent: public AsyncEvent {
public:
    explicit TorchAsyncEvent(std::shared_ptr<torch::Event> event): event_(std::move(event)) {}
...
public:
    ...
    AsyncEventPtr attentionEvent() const override {
...
private:
    ...
    int64_t                          request_id_;
    AsyncEventPtr                    attention_event_;
};

AsyncEventPtr makeConnectorEvent(const std::shared_ptr<torch::Event>& event) {
    return std::make_shared<TorchAsyncEvent>(event);
}
...
void writeCacheToConnector(const CacheStoreInputs& param, IKVCacheConnectorCoordinator* connector_coordinator) {
    ...
    for (size_t batch_id = 0; batch_id < param.context_batch_size; batch_id++) {
        ...
        for (size_t index = 0; index < total_block_num; index++) {
            ...
            block_ids.setAt(index, block_id);
        }

        auto event = param.pre_created_event ? param.pre_created_event : runtimeCreateEvent();
        auto layer_context =
            std::make_shared<WriteCacheLayerContext>(kv_cache_resource, request_id, makeConnectorEvent(event));
