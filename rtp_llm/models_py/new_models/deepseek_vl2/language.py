...
def _resolve_scoring_func(value: Any) -> int:
    ...
    value = _nonnegative_int(value, "scoring_func")
    if value not in (0, 1):
...
def _extract_config_values(
    ...
    q_lora_rank = (
        ...
        else _nonnegative_int(q_lora_rank_value, "q_lora_rank")
    )
    kv_lora_rank_value = raw_config.get("kv_lora_rank", 512)
    kv_lora_rank = (
        0
        if kv_lora_rank_value is None
        else _nonnegative_int(kv_lora_rank_value, "kv_lora_rank")
    )
    nope_head_dim = _nonnegative_int(
        raw_config.get("qk_nope_head_dim", 128), "qk_nope_head_dim"
    )
    rope_head_dim = _nonnegative_int(
        raw_config.get("qk_rope_head_dim", 64), "qk_rope_head_dim"
    )
    v_head_dim = _nonnegative_int(raw_config.get("v_head_dim", 128), "v_head_dim")
    if use_mla:
        if kv_lora_rank == 0:
    ...
    n_shared_experts = _positive_int(
    ...
    moe_layer_freq = _positive_int(
        raw_config.get("moe_layer_freq", 1), "moe_layer_freq"
    )
    moe_layer_index = [
        layer_idx
    ...
    n_group = _positive_int(raw_config.get("n_group", 1), "n_group")
    topk_group = _positive_int(raw_config.get("topk_group", 1), "topk_group")
